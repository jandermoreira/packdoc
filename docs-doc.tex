...
\begin{example}{}
    The \PackageName{article} class can use \Argument{options} (see \Cref{sec:options}). The paper size is one of these \Argument[argument color = blue]{options}.
\end{example}
...
To instance an element, the \MacroRef{DocsCreateElement} macro must be used.

\DocsCreateElement{EnumItemOption}{
    color = red!75!black,
...
\begin{Macrodef}{DocsCreateElement}{\MArg{element name}\MArg{element options}}{}
    ...
    \begin{latexcode}
        \DocsCreateElement{EnumItemOption}{color = red!75!black}
    \end{latexcode}
...
\begin{Macro*}{\Argument{element name}}{\OArg{options}\MArg{item}}{}
    A macro named after \Argument{element name} is created to typeset \Argument{item} in a consistent way. The appearance will follow that defined when \Argument{element name} was created with \Macro{DocsCreateElement}, but can be overridden with \Argument{options} (see \Cref{sec:options}).
\end{Macro*}
...
\begin{Macro*}{\Argument{element name}Def}{\OArg{options}\MArg{item}}{}
    ...
    The appearance will follow that defined when \Argument{element name} was created with \Macro{DocsCreateElement}, but can be overridden with \Argument{options} (see \Cref{sec:options}).
...
An element can be defined, as previously stated, by calling \Macro{\Argument{element name}Def}. This is handy for inline definitions. An alternative way to define an element is to use an environment also created by \MacroRef{DocsCreateElement}. This environment is named \Environment{\Argument{element name}def}.
...
Several options can be used to customize each element. These options are typically specified when the element is created with \MacroRef{DocsCreateElement}, but can also be modified with \MacroRef{DocsSetElement}. Options not specified at creation assume predefined default values, which can also be changed with \MacroRef{DocSet}.

\begin{Macrodef}{DocsSetElement}{\MArg{element name}\MArg{option list}}{}
    After created with \MacroRef{DocsCreateElement}, options can be changed \textit{a posteriori} with \Macro{DocsSetElement}
\end{Macrodef}

\begin{example}{}
    \DocsCreateElement{MyItem}{color = magenta, no single index, no group index}
